    ...
    for filei in file_list:
        ...
        # --------------------------------------------------------------
            ...
            DS['pfull'].attrs['long_name'] = (
                f'(MODIFIED POST-PROCESSING) {DS['lev'].attrs['FIELDNAM']}'
                )
        ...
        # --------------------------------------------------------------
            ...
            var_latu = [var for var in DS.variables if 'latu' in DS[var].dims]
            var_lonv = [var for var in DS.variables if 'lonv' in DS[var].dims]
            
            print(f"From latu to lat: {', '.join(var_latu )}")
            
            for var_name in var_latu:
                # Loop through and unstagger. dims_list finds the dims 
                # of the variable & replaces west_east_stag w/ west_east
            ...
            print(f"From lonv to lon: {', '.join(var_lonv )}")
            
            for var_name in var_lonv:
                var = getattr(DS, var_name)
                dims = var.dims
                longname_txt = var.long_name
                units_txt = var.units
        ...
        if DS[model.areo].dims != (model.time,scalar_axis):
            ...
            DS[model.areo].attrs['long_name'] = (
                f'(SCALAR AXIS ADDED POST-PROCESSING) '
                f'{DS[model.areo].attrs['long_name']}'
                )
        ...
        if args.bin_average:
            ...
            # Loop through coarsened grid, slicing time dimension in 
            # 5-sol increments
            ...
            # Replace  time dimension with the time dimension from DS_average
