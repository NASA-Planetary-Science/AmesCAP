...
def main():
    ...
    for file_name in file_list:
        ...
        DS = xr.open_dataset(input_file_name, decode_times=False)
        ...
        for var in archive_vars.keys():
            ...
            data_vars[var].attrs['long_name'] = archive_vars[var][2]
            data_vars[var].attrs['units'] =  archive_vars[var][3]

        # Create new DataFrame
        DF = xr.Dataset(data_vars, coords=coords)

        # Add longname and unit attibutes to the coordiate variables
        for var in archive_coords.keys():
            DF[var].attrs['long_name'] = archive_coords[var][0]
            DF[var].attrs['units'] = archive_coords[var][1]
