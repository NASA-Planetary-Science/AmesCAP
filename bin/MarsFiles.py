            ...
            for ivar in var_list:
                ...
                if tod_name in varNcf.dimensions and ivar not in [tod_name, 'areo'] and len(varNcf.shape) > 2:
                    ...
                    if parser.parse_args().reconstruct:
                        ...
                        fnew.log_variable("%s_amp" % (ivar), amp.swapaxes(
                            0, 1), new_dim, "tidal amplitude for %s" % (longname_txt), units_txt)
                        fnew.log_variable("%s_phas" % (ivar), phas.swapaxes(
                            0, 1), new_dim, "tidal phase for %s" % (longname_txt), 'hr')
                    ...
                    if parser.parse_args().reconstruct:
                        # time_of_day is the same size as in the original file
                        prCyan("Copying axis: %s..." % (ivar))
                        fnew.copy_Ncvar(fdiurn.variables['areo'])
                    else:
                        prCyan("Processing: %s ..." % (ivar))
                        ...
                        fnew.log_variable(
                            ivar, areo_new, new_dim, varNcf.long_name, varNcf.units)
    ...
    # ===========================================================================
        ...
        for filei in file_list:
            ...
            for ivar in var_list:
                ...
                # Ignore variables like  'time_bounds', 'scalar_axis', and 'grid_xt_bnds'...
                    ...
                    fnew.log_variable(
                        ivar, var_OUT, varNcf.dimensions, longname_txt, units_txt)
                    fnew.log_variable(
                        ivar, var_OUT, varNcf.dimensions, varNcf.long_name, varNcf.units)
            fnew.close()
            fNcdf_t.close()
