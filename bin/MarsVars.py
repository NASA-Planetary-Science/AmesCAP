...
def process_add_variables(file_name, add_list, master_list, debug=False):
    ...
    def add_with_dependencies(var):
        """
        Helper function to add variable and dependencies to the add list.

        :param var: Variable to add
        :type  var: str
        :return: True if the variable and its dependencies can be added,
            False otherwise
        :rtype:  bool
        :raises ValueError: If the input dimensions are not compatible
        :raises TypeError: If the input types are not compatible
    ...
    for input_file in file_list:
        ...
        class FileWrapper:
            def __init__(self, name):
                """
                Initialize the FileWrapper with a file name.
