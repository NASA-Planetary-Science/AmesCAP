...
def main():
    ...
    if len(sys.argv) == 1 or args.command == 'help' or '-h' in sys.argv or '--help' in sys.argv:
        print("""
...
Then, change to the directory hosting your netCDF output files and pass an argument to a CAP executable to perform the operation. A good place to start is to use the example command shown below your desired operation.

Available commands:
  MarsCalendar.py  - Converts Ls into day-of-year (sol) and vice versa.
  MarsFiles.py     - Functions for manipulating entire files (e.g., time-shifting, binning, regridding, temporal and spatial filtering, tide analysis, zonal averaging, etc.)
  MarsFormat.py    - Transforms non-MGCM model output into MGCM-like model output for compatibility with CAP.
  ...
  MarsPull.py      - Query data from the MGCM repository on the NASA NAS Data Portal (data.nas.nasa.gov/mcmc).
  MarsVars.py      - Performs variable manipulations (e.g., deriving secondary variables, column-integration, etc.)
