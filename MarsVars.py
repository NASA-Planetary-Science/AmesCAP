...
C_ext = 12114.286 # Constant for extinction calculation
#===========================
...
def compute_dzTau(q,temp,lev):
    """
    Return dust extinction in [km-1]
    """
        ...
        for ivar in add_list:
            if ivar not in VAR.keys():
                ...
                try:
                    ...
                    if ivar=='dzTau':
                        if 'dst_mass' in fileNC.variables.keys():
                            q=fileNC.variables['dst_mass'][:]*1.e6
                            print(f'dst_mass in file\n{ifile} ... {interp_type}')
                        elif 'dst_mass_micro' in fileNC.variables.keys():
                            q=fileNC.variables['dst_mass_micro'][:]*1.e6
                            print('dst_mass_micro in file')
                        OUT=compute_dzTau(q,temp,lev)
