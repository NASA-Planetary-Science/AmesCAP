...
def vinterp(varIN, Lfull, Llev, type_int="log", reverse_input=False,
    ...
    for k in range(0, Nlev):
        ...
        #     if n[ii] < Nfull-1:
        #         alpha = (np.log(Llev[k] / Lfull[n[ii] + 1, ii])
        #                  / np.log(Lfull[n[ii], ii] / Lfull[n[ii] + 1, ii]))
        #         varOUT[k, ii] = (varIN[n[ii], ii]*alpha
        #                          + (1-alpha)*varIN[n[ii] + 1, ii])
...
def axis_interp(var_IN, x, xi, axis, reverse_input=False, type_int="lin",
    ...
    for k in range(0, len(index)):
        ...
        if np1 >= len(x):
            ...
            # outside the domain (i.e. ``n`` is the last element and
            # ``n+1`` does not exist
            if modulo is not None:
                ...
                # This will set the interpolated value to NaN in ``xi``
                # as last value as ``x[n] - x[np1] = 0``
                np1 -= 1
...
def sol2ls(jld, cumulative=False):
    ...
    def sol2ls_mod(jld):
        ...
        while (diff > small_value):
            ep = e - (e-ec*math.sin(e)-qq) / (1.0-ec*np.cos(e))
            diff = abs(ep - e)
            e = ep

        eq1 = 2.0 * math.atan(er * math.tan(0.5 * e))
        ...
        for i in range(0, len(jld)):
            ...
            while (diff > small_value):
                ...
                diff = abs(ep - e)
                e = ep
            w[i] = 2.0 * math.atan(er * math.tan(0.5*e))
